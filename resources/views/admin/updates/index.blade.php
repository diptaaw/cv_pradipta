...
@section('page-title', "What's New Updates")

@section('breadcrumb')
    <span>&rarr;</span> <span class="active">Updates</span>
@endsection
...
@section('content')
    ...
    <div class="section-header">
        <h2>Portfolio & Project Updates</h2>
    ...
    @if($updates->isEmpty())
        <div class="admin-card" style="text-align: center; padding: 48px;">
            ...
            <p style="color: rgba(255,255,255,0.4); font-size: 0.92rem; margin-bottom: 16px;">
                No updates found. Create one to display in the "What's New" dropdown.
            </p>
            <a href="{{ route('admin.updates.create') }}" class="btn-primary-cms" style="text-decoration: none;">
                    ...
                    <tbody>
                        @foreach($updates as $update)
                            <tr>
                                ...
                                <td style="max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: rgba(255,255,255,0.6);">
                                    {{ $update->description }}
                                </td>
                                <td>
                                    <span style="font-size: 0.82rem; color: rgba(255,255,255,0.5)">{{ $update->date }}</span>
                                </td>
                                <td>
                                    @if($update->is_pinned)
                                ...
                                <td>
                                    <div class="action-buttons" style="justify-content: flex-end;">
                                        ...
                                        <form method="POST" action="{{ route('admin.updates.destroy', $update) }}" onsubmit="return confirm('Are you sure you want to delete this update?')" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
