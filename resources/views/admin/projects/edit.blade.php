...
@section('page-subtitle', 'Perbarui detail proyek, kelola tag terhubung, dan pilih gambar dari pustaka.')
...
@section('content')
    ...
    <form class="admin-form" method="POST" action="{{ route('admin.projects.update', $project) }}" style="max-width: 1000px;">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $project->category) }}" placeholder="Contoh: Game Development, Photography">

                <label>Tahun Tampilan</label>
                <input type="text" name="year" value="{{ old('year', $project->year) }}" placeholder="Contoh: 2026, 2025-2026">

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                    <div>
                        <label>Tanggal Mulai</label>
                        <input type="date" name="start_date" value="{{ old('start_date', $project->start_date ? $project->start_date->format('Y-m-d') : '') }}">
                    </div>
                    <div>
                        <label>Tanggal Selesai</label>
                        <input type="date" name="end_date" value="{{ old('end_date', $project->end_date ? $project->end_date->format('Y-m-d') : '') }}">
                    </div>
                </div>

                <label>Project Link (Live/Demo)</label>
                <input type="url" name="project_link" value="{{ old('project_link', $project->project_link) }}" placeholder="https://example.com">

                <label>GitHub Link</label>
                <input type="url" name="github_link" value="{{ old('github_link', $project->github_link) }}" placeholder="https://github.com/username/project">
            ...
            <div>
                ...
                <textarea name="description" style="min-height: 140px;">{{ old('description', $project->description) }}</textarea>
                ...
                <div style="display: flex; gap: 20px; margin-top: 24px; margin-bottom: 24px; flex-wrap: wrap;">
                    <label style="display: inline-flex; align-items: center; gap: 8px; margin-bottom: 0; cursor: pointer;">
                        <input type="checkbox" name="featured" value="1" {{ old('featured', $project->featured) ? 'checked' : '' }} style="width: auto; margin-bottom: 0;">
                        Tampilkan sebagai unggulan (Maks 5)
                    </label>

                    <label style="display: inline-flex; align-items: center; gap: 8px; margin-bottom: 0; cursor: pointer;">
                        <input type="checkbox" name="archived" value="1" {{ old('archived', $project->archived) ? 'checked' : '' }} style="width: auto; margin-bottom: 0;">
                        Arsipkan project
                    </label>

                    <label style="display: inline-flex; align-items: center; gap: 8px; margin-bottom: 0; cursor: pointer;">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', $project->is_published) ? 'checked' : '' }} style="width: auto; margin-bottom: 0;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 16px; max-height: 350px; overflow-y: auto; border: 1px solid rgba(255,255,255,0.1); padding: 16px; border-radius: 14px; background: rgba(0,0,0,0.15); margin-bottom: 24px;">
                @foreach($mediaList as $media)
                    @php 
                        ...
                        $isThumbnail = (old('thumbnail', $project->thumbnail) == $path || old('thumbnail', $project->thumbnail) == asset($path)) ? 'checked' : '';
                        $inGallery = is_array($project->gallery_images) && in_array($path, $project->gallery_images) ? 'checked' : '';
                    @endphp
                    <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); padding: 10px; border-radius: 10px; display: flex; flex-direction: column; justify-content: space-between; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 4px; width: 100%;">
                            <label style="display: flex; align-items: center; gap: 6px; font-size: 0.78rem; margin-bottom: 0; color: rgba(255,255,255,0.8); cursor: pointer;">
                                <input type="radio" name="thumbnail" value="{{ $path }}" {{ $isThumbnail }} style="width: auto; margin-bottom: 0;">
                                Thumbnail
                            </label>
                            <label style="display: flex; align-items: center; gap: 6px; font-size: 0.78rem; margin-bottom: 0; color: rgba(255,255,255,0.8); cursor: pointer;">
                                <input type="checkbox" name="gallery_images[]" value="{{ $path }}" {{ $inGallery }} style="width: auto; margin-bottom: 0;">
                                Galeri
                            </label>
                        </div>
                    </div>
