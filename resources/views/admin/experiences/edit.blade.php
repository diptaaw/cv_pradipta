...
@section('breadcrumb')
    ...
    <span>&rarr;</span> <span class="active">Edit Experience</span>
@endsection
        ...
        <div class="cms-form-grid" style="grid-template-columns: 1.2fr 1fr; gap: 24px;">
            ...
            <div class="admin-card cms-card-section">
                ...
                <div class="cms-form-group">
                    ...
                    <input class="cms-input" id="organization" type="text" name="organization" value="{{ old('organization', $experience->organization) }}" required placeholder="e.g. HIMA Multimedia Broadcasting PENS">
                </div>

                <div class="cms-form-group">
                    <label for="title">Position / Role Title</label>
                    <input class="cms-input" id="title" type="text" name="title" value="{{ old('title', $experience->title) }}" required placeholder="e.g. Staff PSDM">
                </div>

                <div class="cms-form-group">
                    <label for="year">Display Period / Year</label>
                    <input class="cms-input" id="year" type="text" name="year" value="{{ old('year', $experience->year) }}" required placeholder="e.g. 2025 — PRESENT">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div class="cms-form-group">
                        <label for="start_date">Start Date</label>
                        <input class="cms-input" id="start_date" type="date" name="start_date" value="{{ old('start_date', $experience->start_date ? $experience->start_date->format('Y-m-d') : '') }}">
                    </div>
                    <div class="cms-form-group">
                        <label for="end_date">End Date</label>
                        <input class="cms-input" id="end_date" type="date" name="end_date" value="{{ old('end_date', $experience->end_date ? $experience->end_date->format('Y-m-d') : '') }}">
                    </div>
                </div>

                <div class="cms-form-group">
                    <label for="position">Display Order Position</label>
                    <input class="cms-input" id="position" type="number" name="position" value="{{ old('position', $experience->position) }}" required>
                </div>

                <div class="cms-form-group">
                    <label for="description">Role Description / Contribution</label>
                    <textarea class="cms-textarea" id="description" name="description" placeholder="Write description of your roles and achievements..." style="min-height: 120px;">{{ old('description', $experience->description) }}</textarea>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="cms-form-group">
                        ...
                        <div style="max-height: 140px; overflow-y: auto; border: 1px solid rgba(255,255,255,0.08); padding: 12px; border-radius: 12px; background: rgba(0,0,0,0.15); display: flex; flex-direction: column; gap: 8px;">
                            @forelse($tags as $tag)
                                @php 
                                    $checked = in_array($tag->id, $experienceTagIds) ? 'checked' : ''; 
                                @endphp
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 16px;">
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; user-select: none;">
                            <input type="checkbox" name="featured" value="1" {{ old('featured', $experience->featured) ? 'checked' : '' }} style="width: 16px; height: 16px; accent-color: #7b61ff; cursor: pointer;">
                            <span style="font-size: 0.85rem; font-weight: 600; color: white;">Featured (Show on Homepage)</span>
                        </label>

                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; user-select: none;">
                            <input type="checkbox" name="is_published" value="1" {{ old('is_published', $experience->is_published) ? 'checked' : '' }} style="width: 16px; height: 16px; accent-color: #7b61ff; cursor: pointer;">
